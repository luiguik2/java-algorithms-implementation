...
public final class ReflectUtils {
    ...
    /**
     * Returns the 0th element of the list returned by {@code getAnnotations}, or {@code null} if the list would be
     * empty.
     *
     ...
     * @return the first annotation found of the specified type which is present on {@code m}, or present on any methods
     * which {@code m} overrides.
     * @throws NullPointerException if any argument is {@code null}.
     * @see ReflectUtils#getAnnotations(Method, Class)
     */
    public static <A extends Annotation> Optional<A> getAnnotation(Method m, Class<A> t) {
     ...
     * <p>
     * Returns a list of all of the annotations of the specified type which are either present on {@code m}, or present
     * on any methods declared by a supertype of {@code D} which {@code m} overrides.
     * <p>
     * Annotations are listed in order of nearest proximity to {@code D}, that is, assuming {@code D extends E} and
     * {@code E extends F}, then the returned list would contain annotations in the order of {@code [D, E, F]}. A bit
     * more formally, if {@code Sn} is the nth superclass of {@code D} (where {@code n} is an integer starting at 0),
     * then the index of the annotation present on {@code Sn.m} is {@code n+1}, assuming annotations are present on
     * {@code m} for every class.
     * <p>
     * Annotations from methods declared by the superinterfaces of {@code D} appear <em>last</em> in the list, in order
     * of their declaration, recursively. For example, if {@code class D implements X, Y} and {@code interface X extends
     * Z}, then annotations will appear in the list in the order of {@code [D, X, Z, Y]}.
     *
     * @param <A> the type of the annotation to find.
     * @param m   the method to begin the search from.
     * @param t   the type of the annotation to find.
     * @return a list of all of the annotations of the specified type which are either present on {@code m}, or present
     * on any methods which {@code m} overrides.
     * @throws NullPointerException if any argument is {@code null}.
     */
...
//        }
    ...
    public static boolean overrides(Method a, Method b) {
        ...
        if ((isPackageAccess(modsA) || isPackageAccess(modsB)) && !Objects
                .equals(classA.getPackage(), classB.getPackage())) {
            return false;
        }
    ...
    private static final List<Integer> ACCESS_ORDER = Arrays
            .asList(Modifier.PRIVATE, 0, Modifier.PROTECTED, Modifier.PUBLIC);

    public static int compareAccess(int lhs, int rhs) {
        return Integer
                .compare(ACCESS_ORDER.indexOf(lhs & ACCESS_MODIFIERS), ACCESS_ORDER.indexOf(rhs & ACCESS_MODIFIERS));
    }
}
